...
void * pasarProcesosEstadoAExit(void * sin_parametro) {
    ...
    while(1) {
        ...
        char * mensaje_finalizacion = string_from_format("Finaliza el proceso %d - Motivo: %s", pcb_fin->contexto->PID, pcb_fin->motivo_finalizacion);
...
pcb * seleccionarProcesoPorAlgoritmo(void) { //Devuelve el proceso seleccionado por el algoritmo 
    ...
    case FIFO: return obtenerProcesoPorFIFO();        
    case RR: return obtenerProcesoPorRR();
    case VRR: return obtenerProcesoPorVRR();
    default:
...
pcb * obtenerProcesoPorVRR(void) {
    pcb * proceso_elegido;
    if(colaSincronizadaEstaVacia(cola_ready_vrr)){ // SI LA COLA DE PROCESOS CON QUANTUM ESTA VACIA
...
void operarSegunMotivoDesalojo(t_paquete * proceso_desalojado_paq, int conexion_cpu_dispatch) {
    ...
    switch (motivo){
        case P_EXIT:
            ...
            if(algoritmo_elegido != FIFO){ pthread_cancel(hilo_timer); }
            verSiPararPlanificacion();
        ...
        case INT_FIN_QUANTUM:
            ...
            char * mensaje_fin_q = string_from_format("PID: %d - Desalojado por fin de Quantum", proceso_recibido->PID);
...
void hacerOperacionIO(t_paquete * paquete_io) {
    ...
    if(!existeInterfazEnSistema(nombre_interfaz) || 
        !interfazPuedeRealizarOperacion(paquete_io->codigoOperacion, nombre_interfaz)) {
        verSiPararPlanificacion();
...
// Todas las interfaces de I/O tendrán un único nombre que las identificará en el sistema, este nombre deberá ser consultado al momento de que una Interfaz se conecte al kernel. 
// Como este nombré será único se utilizará en los programas para identificar a la Interfaz.
...
// -------------------------- operar Segun Motivo Desalojo Wait Signal -------------------------- 
// -------------------------- Manejo de Recursos -------------------------- 
...
// A la hora de recibir de la CPU un Contexto de Ejecución desalojado por WAIT, el Kernel deberá verificar primero que exista el recurso solicitado y en caso de que exista restarle 1 a la cantidad de instancias del mismo. 
// En caso de que el número sea estrictamente menor a 0, el proceso que realizó WAIT se bloqueará en la cola de bloqueados correspondiente al recurso.
...
// A la hora de recibir de la CPU un Contexto de Ejecución desalojado por SIGNAL, el Kernel deberá verificar primero que exista el recurso solicitado, luego sumarle 1 a la cantidad de instancias del mismo. 
